...
我认为应该返回特殊值，null表示未获取到主机名。

本机名在这个场景下应该是一个关键信息，之后追溯联路可能会需要主机名信息。但是id生成应该与业务完全无关，不能因为id生成失败而导致业务失败（或单独适配对应的异常）。

异常情况下没有本机名可能导致联路信息不完整，但是为了更低的入侵性，我认为是可以接受的。



相关文章：https://blog.51cto.com/u_12929/6509119

可检查异常：可检查异常(除了Exception中的RuntimeException及其子类以外，其他的Exception类及其子类)

在编译期间会强制你try-catch或者throws出去



不可检查异常：RuntimeException及其子类。我们可以灵活地选择在某个函数中集中处理，比如在Spring中的AOP切面中集中处理异常。

例如寻源切面处理。

```java
package com.alibaba.industry.sourcing.infrastructure.aop;
...
import com.alibaba.industry.sourcing.api.base.PageResult;
import com.alibaba.industry.sourcing.api.enums.ErrorCodeEnum;
import com.alibaba.industry.sourcing.domain.base.exception.BizException;
import com.alibaba.industry.sourcing.domain.base.exception.ContentCsiException;
import com.alibaba.industry.sourcing.domain.base.exception.ParamException;
import com.alibaba.industry.sourcing.domain.base.exception.SysException;
import com.alibaba.industry.sourcing.domain.base.framework.repository.exception.EntityNotExistException;
import com.alibaba.industry.sourcing.domain.base.framework.repository.exception.EntityUpdateConflictException;
import com.alibaba.industry.sourcing.domain.quotation.exception.QuotationCreateValidateException;
import com.alibaba.industry.sourcing.domain.quotation.exception.QuotationPermissionValidateException;
import com.alibaba.industry.sourcing.infrastructure.dal.search.rfq.RfqOpensearchExcetion;
import com.alibaba.industry.sourcing.infrastructure.lock.LockException;
import com.alibaba.industry.sourcing.infrastructure.util.DingTalkRobotUtils;
import com.alibaba.industry.sourcing.infrastructure.util.PageResultGenerator;
import com.alibaba.industry.sourcing.infrastructure.util.ResultGenerator;
 ...
 * <p>
 * 一点点建议：
 * 1. 应用内部调用的服务（比如提供CURD能力的Basic层服务）建议使用异常，对外暴露的RPC服务建议使用Result
 * 2. 不建议既使用Result又使用异常（特别是catch异常），例: Result foo() throws Exception
 */
@Aspect
@Component
...
public class ExceptionHandler {
    /**
     * 异常日志统一存储、收集，配置见 logback-spring.xml
     */
    private static final Logger LOG = LoggerFactory.getLogger("ExceptionHandler");
    private static final Logger BIZ_MONITOR_LOG = LoggerFactory.getLogger("BizMonitor");

    /**
     * 省略异常处理代码时，请确认服务在该切入点的切入范围内。
     *
     * @param point
     * @return Result
     */
    @Around(
        "execution(com.alibaba.industry.sourcing.api.base.Result com.alibaba.industry.sourcing..*(..)) "
            + "|| execution(com.alibaba.industry.sourcing.api.base.PageResult com.alibaba.industry.sourcing..*(..))")
    public Object handle(ProceedingJoinPoint point) throws Throwable {
        try {
            Object returnVal = point.proceed();
            bizMonitorOnSuccess(point);
            return returnVal;
        } catch (Throwable exception) {
            rollbackTransaction();
            bizMonitorOnException(point, exception);
            ...
            LOG.error(String.format("%s#%s exception, args: %s, trace-id :%s",
                targetClassName, targetMethodName, args, traceId), exception);
            
            if (exception instanceof EntityNotExistException) {
                return genError(point, ErrorCodeEnum.DATA_NOT_EXIST);
            }
            if (exception instanceof EntityUpdateConflictException) {
                return genError(point, ErrorCodeEnum.UPDATE_FAILED);
            }
            if (exception instanceof ParamException) {
                return genError(point, ErrorCodeEnum.INVALID_PARAM, exception.getMessage());
            }
            if (exception instanceof QuotationCreateValidateException) {
                return genError(point, ((QuotationCreateValidateException)exception).getCode(), exception.getMessage());
            }
            if (exception instanceof QuotationPermissionValidateException) {
                return genError(point, ErrorCodeEnum.NO_PERMISSION);
            }
            if (exception instanceof RfqOpensearchExcetion) {
                return genError(point, ErrorCodeEnum.QUERY_OPENSEARCH_ERROR);
            }
            if (exception instanceof LockException) {
                return genError(point, ErrorCodeEnum.SYSTEM_TRY_LOCK_FAILED);
            }
            if (exception instanceof BizException) {
                String errorCode = ((BizException)exception).getCode();
                if (StringUtils.isBlank(errorCode)) {
                    errorCode = ErrorCodeEnum.BIZ_ERROR.getErrorCode();
                }
                return genError(point, errorCode, exception.getMessage());
            }
            if (exception instanceof SysException) {
                return genError(point, ErrorCodeEnum.SYSTEM_ERROR, exception.getMessage());
            }
            if (exception instanceof BlockException) {
                return genError(point, ErrorCodeEnum.BLOCK_ERROR);
            }
            if (exception instanceof ContentCsiException) {
                return genError(point, ErrorCodeEnum.CONTENT_CSI);
            }
            DingTalkRobotUtils.sendMessage(String.format("调用 %s#%s 发生未知异常，请根据TRACE-ID查看日志排查，入参：%s, 异常信息：%s",
                targetClassName, targetMethodName, args, exception.getMessage()));
            return genError(point, ErrorCodeEnum.SYSTEM_ERROR);
        }
    }

    private boolean isCglibProxyClass(ProceedingJoinPoint point) {
        String className = point.getTarget().getClass().getName();
        return StringUtils.contains(className,"$$EnhancerBySpringCGLIB");
    }
    ...
    private Object genError(ProceedingJoinPoint point, ErrorCodeEnum errorCodeEnum) {
        return genError(point, errorCodeEnum, errorCodeEnum.getErrorMsg());
    }

    private Object genError(ProceedingJoinPoint point, ErrorCodeEnum errorCodeEnum, String customMsg) {
        Method targetMethod = getInvokeMethod(point);
        Class<?> returnType = targetMethod.getReturnType();
        if (PageResult.class.isAssignableFrom(returnType)) {
            return PageResultGenerator.genError(errorCodeEnum, customMsg);
        }
        return ResultGenerator.genError(errorCodeEnum, customMsg);
    }

    private Object genError(ProceedingJoinPoint point, String errorCode, String customMsg) {
        Method targetMethod = getInvokeMethod(point);
        Class<?> returnType = targetMethod.getReturnType();
        if (PageResult.class.isAssignableFrom(returnType)) {
            return PageResultGenerator.genError(errorCode, customMsg);
        }
        return ResultGenerator.genError(errorCode, customMsg);
    }

    private void bizMonitorOnSuccess(ProceedingJoinPoint point) {
        if(isCglibProxyClass(point)){
            return;
        }
        Method targetMethod = getInvokeMethod(point);
        BizMonitor bizMonitorAno = AnnotationUtils.findAnnotation(targetMethod, BizMonitor.class);
        bizMonitor(bizMonitorAno, "success");
    }

    private void bizMonitorOnException(ProceedingJoinPoint point, Throwable e) {
        if(isCglibProxyClass(point)){
            return;
        }
        Method targetMethod = getInvokeMethod(point);
        ...
        if (e instanceof BizException) {
            bizMonitor(bizMonitorAno, "warn");
        } else {
            bizMonitor(bizMonitorAno, "error");
        }
    }

    private Method getInvokeMethod(ProceedingJoinPoint point) {
        Signature signature = point.getSignature();
        MethodSignature methodSignature = (MethodSignature)signature;
        return methodSignature.getMethod();
    }

    private void bizMonitor(BizMonitor bizMonitor, String level) {
        if (bizMonitor != null) {
            BIZ_MONITOR_LOG.info("[业务监控] {} {}", bizMonitor.value(), level);
        }
    }
}
