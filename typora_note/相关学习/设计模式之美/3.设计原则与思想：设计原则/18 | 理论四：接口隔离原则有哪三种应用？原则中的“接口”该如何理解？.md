...
说到原子类就必须提到一个概念：原子性。`getAndIncrement` 的目的是为了获取当前值并对当前值执行+1操作，单线程中乍一看确实可以拆成两个方法，但是在并发场景下由于CPU时间片调度的不确定性以及指令重排等因素，很有可能Thread1还没来的及执行+1操作，Thread2就执行了Get操作，导致结果不符合预期而破坏了接口本身的功能定义，所以`getAndIncrement`必须这样来设计，并不违反隔离原则。