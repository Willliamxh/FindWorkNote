...
![](/Users/xuhan/Documents/workNote/FindWorkNote/typora_note/相关学习/设计模式之美/3.设计原则与思想：设计原则/22 | 理论八：如何用迪米特法则（LOD）实现“高内聚、松耦合”？.assets/62275095f1f5817cad8a9ca129a6ec3c.jpg)
...
“高内聚、松耦合”指相近的功能放在同一个类，不相近的功能放在多个类，保证类的独立性。

“单一职责原则”是指类/接口的职责尽量单一。

“接口隔离原则”提供了一种判定接口是否职责单一的标准，调用者如果只使用部分接口，接口设计就不够职责单一。

“基于接口而非实现编程”将接口和实现分离，封装不稳定的实现，暴露稳定的接口。

“迪米特法则”不该有依赖关系的类之间，不要有依赖，有依赖关系的类之间，尽量只依赖必要的接口

联系：

这几个原则，设计的目的主要是为了应对功能变化时，尽可能只修改较少的类，减少对所依赖的类的影响，提高代码的扩展性。