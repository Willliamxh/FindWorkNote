...
总结：控制反转不是一种具体的实现技巧，而是指导框架层面的设计的思想。DI是控制反转的一种实现方式



区别：基于接口而非实现编程，是一种编程指导思想，依赖注入是一种编程技巧。

联系：使用依赖注入时，一般将要注入的对象设计为接口，利于拓展。





1，依赖注入是指把依赖的类通过方法参数或者构造函数参数传递，是一种编程技巧，而不是通过new的方式。传入的参数可以是申明的接口或者类，可以实现解耦，提高扩展性

2，基于接口而非实现编程是接口和实现分离，是一种思想，体现封装特性的原则。解耦，提高扩展性





依赖注入在实际执行过程中注入的是具体的实现，而在声明依赖时可以声明依赖为接口也可以声明依赖为类，当声明依赖为接口时可以理解为业务代码逻辑的编写是一种"基于接口而非实现编程"；而声明依赖为类时则不满足这个逻辑了。以Spring为例，通过@Autowired 注入的结果为类的实现Component，该Component可以为声明依赖的类类型或者是Component继承或者实现的接口。

声明依赖为接口的方式可以便于对实现进行替换，而如果声明依赖为具体类时替换逻辑要更复杂些。

在实际开发场景中如果只是一种简单依赖可以直接依赖类实现，如果是较为复杂的系统并且涉及到多种"类实现"对应的替换时则"基于接口编程"可以降低类实现替换的复杂度