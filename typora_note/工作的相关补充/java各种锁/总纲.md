...
![image-20200419213508934](C:\work\courses\公开课\面试题\你以为你真的懂Synchronized\lock_step.png)![lock_step](synchronized.assets/lock_step.png)

![](C:\work\courses\公开课\面试题\你以为你真的懂Synchronized\markword-64.png)

![markword-64](synchronized.assets/markword-64.png)
...
线程可见性：吧这个值那倒我的线程内存里面，如果没有加volatile的时候，只是再我线程的内存里面改了，没有到主存里面去改。



volatile作用：我在一个线程里面改了一个值，我在另外一个线程里面能看到这个修改的值
...
cpu再执行的时候都是流水线化执行，如果12没有关系 1执行慢 2执行快 那就是2先执行。那我就乱序执行。

1.乱序会产生严重后果嘛？会



<img src="总纲.assets/image-20210308104009987.png" alt="image-20210308104009987" style="zoom:33%;" />

new一个对象的时候

![image-20210308104205942](总纲.assets/image-20210308104205942.png)
...
饿汉式：不管怎么样 都创建。 我都没用 都占用内存空间了 但是这个都是安全的

![image-20210316132334649](总纲.assets/image-20210316132334649.png)

懒汉式：我们时候我需要了 再调用这个对象。 但是这个多线程访问会造成不一致，第一个线程来了 是空的，第二个线程进来 还是空的，然后new了两个对象出来。

解决：synchronize给懒汉式对象上锁  问题：我给这个锁的粒度太粗了，把一些不用加锁的都给锁住了。

![image-20210316132828614](总纲.assets/image-20210316132828614.png)

解决：锁细化，那就是当instance为空的时候，

![image-20210308105156205](总纲.assets/image-20210308105156205.png)

再锁，但是还是会出现之前一样的问题。 两个线程 都遇到了instance为空。第一个线程先创建 拿到锁，创建；第二个现场又拿到了这个锁。

解决：double check lock 

![image-20210308105047529](总纲.assets/image-20210308105047529.png)

我先判断你是不是为空，然后上锁的时候 再判断你是不是为空，有点像cas。



问题：这个单例需不需要加volatile？

<img src="总纲.assets/image-20210308105428201.png" alt="image-20210308105428201" style="zoom:50%;" />

结论：一定要加

![image-20210308105523703](总纲.assets/image-20210308105523703.png)
...
![image-20210308105554019](总纲.assets/image-20210308105554019.png)

![image-20210308105607160](总纲.assets/image-20210308105607160.png)

![image-20210308105615804](总纲.assets/image-20210308105615804.png)

一个线程来的时候，做了半初始化（赋值为0）。这个时候发生了指令重排，t指向了一个半初始化对象。

那我第二个线程来的时候。

![image-20210308105731429](总纲.assets/image-20210308105731429.png)

由于t已经不为空了，直接把这个对象哪来用了，这个时候就用到了半初始化的对象。这不是我们想要的。



### 那加了volatile就能解决这个问题嘛？

可以

如何解决的？

JVM对volatile修饰的语句，让她不能指令重排序。 实现方式 内存屏障

<img src="总纲.assets/image-20210308110138905.png" alt="image-20210308110138905" style="zoom:50%;" />

读读不能互换，内存屏障。

volatile  底层的一个实现：Lock addl 

<img src="总纲.assets/image-20210308110304958.png" alt="image-20210308110304958" style="zoom:50%;" />



jvm规定上，都应该实现这四种屏障。读写的时候都要加。这些都是规范的要求。

<img src="总纲.assets/image-20210308110550474.png" alt="image-20210308110550474" style="zoom:50%;" />

有哪些指令不可以重排序？8种情况 不能重排序

![image-20210308110648383](总纲.assets/image-20210308110648383.png)

JVM层面：对写了volatile的代码段加了个屏障；规定了8种情况加了屏障。



那具体的实现是什么？

synchronize  lock cmpxchg
...
![image-20210308132125361](总纲.assets/image-20210308132125361.png)

<img src="总纲.assets/image-20210308132138625.png" alt="image-20210308132138625" style="zoom: 50%;" />

![image-20210308132231510](总纲.assets/image-20210308132231510.png)

几个核心 共享一个L3 但是每一个核心 都有自己的l1和l2



按块来读内存，按照块来读，是一个cacheline

![image-20210308132505240](总纲.assets/image-20210308132505240.png)

<img src="总纲.assets/image-20210308132532033.png" alt="image-20210308132532033" style="zoom:50%;" />

cacheline ：64个字节

大小是怎么确定的？

![image-20210308132616716](总纲.assets/image-20210308132616716.png)
...
2.两个cpu分别调用了一行里面的不同的数据

3.那其中一个cpu修改了一个数据，那其实 我得通知另外一个cpu说，你得去另外一个地方再取一遍。

4.这个情况下，就是使用了一个协议：缓存一致性

![image-20210308133443129](总纲.assets/image-20210308133443129.png)

缓存行四种状态：

modify 被修改  exclusive被独占   shared 共享了  invalid 失效了

那我一个cpu对缓存行进行了 修改  那就是要通知别的缓存航 失效。你得去内存里面重新读。



伪共享：

两个数据在同一行，会导致速度比较慢。因为用了volatile 再同一个块里面 然后你在底层用不同cpu去读取的时候，会不断的通知，我这个数值修改了，你得去 内存里面重新读取

<img src="总纲.assets/image-20210308134221653.png" alt="image-20210308134221653" style="zoom:33%;" />

<img src="总纲.assets/image-20210308134403556.png" alt="image-20210308134403556" style="zoom:33%;" />



这个时候，不再同一行。那就减少了通知的过程

<img src="总纲.assets/image-20210308134205031.png" alt="image-20210308134205031" style="zoom:50%;" />

什么加了这么多数据后 反而快了。

因为不加数据的时候，可能在同一个块里面，你在修改的时候，会不断的通知别的CPU，你得更新 你得

而再不同块里面的时候，就不需要通知了。不会有太多的失效



消息队列：头尾相联的数组。ringBuffer  初始化的时候 有很多p1到p7  为了解决缓存行的伪共享问题。



jdk 1.8 用了个注解 保证 我的的变量和别人不再同一行 ![image-20210308134941550](总纲.assets/image-20210308134941550.png)
...
通过那个加long数据的就能进行证明。 因为不用相互通知失效。

# 用hsdis观察synchronized和volatile

1. 安装hsdis (自行百度) hotspot的反汇编器
...
凡事有一段代码执行次数超多的时候，那就编译成asm，下次直接执行asm，这种代码叫热点代码 hotspot。
