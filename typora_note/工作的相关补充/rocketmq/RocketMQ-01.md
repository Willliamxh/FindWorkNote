...
- 缺点：单台机器宕机期间，这台机器上未被消费的消息在机器恢复之前不可订阅，消息实时性会受到影响。（因为没有消息的备份了）
...
- 缺点：Master宕机，磁盘损坏情况下会丢失少量消息。（因为可能来不及复制就宕机了）

什么叫异步？

当producer发送消息给broker的之后，broker得落盘。

异步就是说，当producer发消息发到broker master 落盘之后，就和producer说，“啊，我已经收到你发的消息了“

然后异步得将数据复制给broker slave中 落盘

什么是同步？

那就是说，我消息发到broker master 落盘之后，先不和producer说，先去将数据复制给broker slave中 落盘，

slave复制落盘结束后，再去给producer发消息说，我收到你发的消息了

什么时候是同步？一定要保证一致性很强，对消息安全性要求很高 

![image-20220628215806999](RocketMQ-01.assets/image-20220628215806999.png)

#### 4）多Master多Slave模式（同步）

每个Master配置一个Slave，有多对Master-Slave，HA（双机集群(HA)系统简称）采用同步双写方式，即只有主备都写成功，才向应用返回成功，这种模式的优缺点如下：

- 优点：数据与服务都无单点故障，Master宕机情况下，消息无延迟，服务可用性与数据可用性都非常高；
- 缺点：性能比异步复制模式略低（大约低10%左右），发送单个消息的RT会略高，且目前版本在主节点宕机后，备机不能自动切换为主机。（因为数据同步需要时间）
...
| **序号** | **IP**       | **角色**                 | **架构模式**    |
| -------- | ------------ | ------------------------ | --------------- |
| 1        | 10.211.55.13 | nameserver、brokerserver | Master1、Slave2 |
| 2        | 10.211.55.12 | nameserver、brokerserver | Master2、Slave1 |
...
```bash
# nameserver
10.211.55.13 rocketmq-nameserver1
10.211.55.12 rocketmq-nameserver2
# broker
10.211.55.13 rocketmq-master1
10.211.55.13 rocketmq-slave2
10.211.55.12 rocketmq-master2
10.211.55.12 rocketmq-slave1
```
...
unbuntu：service network-manager restart
...
http://www.win7zhijia.cn/win10jc/win10_45243.html
...
```bash
#set rocketmq
ROCKETMQ_HOME=/opt/rocketmq/rocketmq-4.9.3
...
```bash
mkdir /opt/rocketmq/rocketmq-4.9.3/store
mkdir /opt/rocketmq/rocketmq-4.9.3/store/commitlog
mkdir /opt/rocketmq/rocketmq-4.9.3/store/consumequeue
mkdir /opt/rocketmq/rocketmq-4.9.3/store/index




mkdir /opt/rocketmq/rocketmq-4.9.3/store1
mkdir /opt/rocketmq/rocketmq-4.9.3/store1/commitlog
mkdir /opt/rocketmq/rocketmq-4.9.3/store1/consumequeue
mkdir /opt/rocketmq/rocketmq-4.9.3/store1/index


mkdir /opt/rocketmq/rocketmq-4.9.3/store2
mkdir /opt/rocketmq/rocketmq-4.9.3/store2/commitlog
mkdir /opt/rocketmq/rocketmq-4.9.3/store2/consumequeue
mkdir /opt/rocketmq/rocketmq-4.9.3/store2/index



mkdir /opt/rocketmq/rocketmq-4.9.3/store3
mkdir /opt/rocketmq/rocketmq-4.9.3/store3/commitlog
mkdir /opt/rocketmq/rocketmq-4.9.3/store3/consumequeue
mkdir /opt/rocketmq/rocketmq-4.9.3/store3/index
...
### 3.3.7.2 然后重复对另一台服务器也进行相应的操作

这边千万不要手贱去创建什么about目录，问题相关：

https://github.com/apache/rocketmq/issues/1609

查看报错日志：

/home/xuhan/logs/rocketmqlogs
...
```sh
vi /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-a.properties
```
...
#存储路径
storePathRootDir=/opt/rocketmq/rocketmq-4.9.3/store
#commitLog 存储路径
storePathCommitLog=/opt/rocketmq/rocketmq-4.9.3/store/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/opt/rocketmq/rocketmq-4.9.3/store/consumequeue
#消息索引存储路径
storePathIndex=/opt/rocketmq/rocketmq-4.9.3/store/index
#checkpoint 文件存储路径
storeCheckpoint=/opt/rocketmq/rocketmq-4.9.3/store/checkpoint
#abort 文件存储路径
abortFile=/opt/rocketmq/rocketmq-4.9.3/store/abort
#限制的消息大小
...
```sh
vi /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-b-s.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/opt/rocketmq/rocketmq-4.9.3/store1
#commitLog 存储路径
storePathCommitLog=/opt/rocketmq/rocketmq-4.9.3/store1/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/opt/rocketmq/rocketmq-4.9.3/store1/consumequeue
#消息索引存储路径
storePathIndex=/opt/rocketmq/rocketmq-4.9.3/store1/index
#checkpoint 文件存储路径
storeCheckpoint=/opt/rocketmq/rocketmq-4.9.3/store1/checkpoint
#abort 文件存储路径
abortFile=/opt/rocketmq/rocketmq-4.9.3/store1/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
```sh
vi /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-b.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/opt/rocketmq/rocketmq-4.9.3/store2
#commitLog 存储路径
storePathCommitLog=/opt/rocketmq/rocketmq-4.9.3/store2/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/opt/rocketmq/rocketmq-4.9.3/store2/consumequeue
#消息索引存储路径
storePathIndex=/opt/rocketmq/rocketmq-4.9.3/store2/index
#checkpoint 文件存储路径
storeCheckpoint=/opt/rocketmq/rocketmq-4.9.3/store2/checkpoint
#abort 文件存储路径
abortFile=/opt/rocketmq/rocketmq-4.9.3/store2/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
```sh
vi /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-a-s.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/opt/rocketmq/rocketmq-4.9.3/store3
#commitLog 存储路径
storePathCommitLog=/opt/rocketmq/rocketmq-4.9.3/store3/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/opt/rocketmq/rocketmq-4.9.3/store3/consumequeue
#消息索引存储路径
storePathIndex=/opt/rocketmq/rocketmq-4.9.3/store3/index
#checkpoint 文件存储路径
storeCheckpoint=/opt/rocketmq/rocketmq-4.9.3/store3/checkpoint
#abort 文件存储路径
abortFile=/opt/rocketmq/rocketmq-4.9.3/store3/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
总结：一组主从分开布置在两台服务器时,对外监听的端口号要一致,一组主从一致,一组注册一致,两个注册监听端口号为两种端口号
...
```sh
vi /opt/rocketmq/rocketmq-4.9.3/bin/runbroker.sh
```
...
```sh
vim /opt/rocketmq/rocketmq-4.9.3/bin/runserver.sh
```
...
分别在10.211.55.13和10.211.55.12启动NameServer

```bash
cd /opt/rocketmq/rocketmq-4.9.3/bin
nohup sh mqnamesrv &
```

#### 2）启动Broker集群

* 在10.211.55.13上启动master1和slave2
* 这边自己敲一下路径
...
```bash
cd /opt/rocketmq/rocketmq-4.9.3/bin
nohup sh mqbroker -c /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-a.properties &
```
...
```sh
cd /opt/rocketmq/rocketmq-4.9.3/bin
nohup sh mqbroker -c /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-b-s.properties &
```

* 在10.211.55.12上启动master2和slave2

master2

```sh
cd /opt/rocketmq/rocketmq-4.9.3/bin
nohup sh mqbroker -c /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-b.properties &
```
...
```sh
cd /opt/rocketmq/rocketmq-4.9.3/bin
nohup sh mqbroker -c /opt/rocketmq/rocketmq-4.9.3/conf/2m-2s-sync/broker-a-s.properties &
```
...
```sh
rocketmq.config.namesrvAddr=10.211.55.13:9876;10.211.55.12:9876
```
...
2.指定Nameserver地址（因为nameserver是管理者 我们需要通过nameserver去找到broker）
3.启动producer（为了发送消息）
...
public class Consumer {
    public static void main(String[] args) throws MQClientException {
        ...
        // 2.指定Nameserver地址
        consumer.setNamesrvAddr("10.211.55.13:9876;10.211.55.12:9876");
        // 3.订阅主题Topic和Tag
        consumer.subscribe("base","tag2");
        // 4.设置回调函数，处理消息 其实就是设定一个监听器
        consumer.registerMessageListener(new MessageListenerConcurrently() {
            //当我们拿到消息之后 就会进入这个方法内部
            //这个就是接收消息内容的方法
            @Override
...
负载均衡=谁的性能好就多消费一点，并不是轮流的意思

![image-20220702211949706](RocketMQ-01.assets/image-20220702211949706.png)
...
![image-20220702211823021](RocketMQ-01.assets/image-20220702211823021.png)
...
下面用订单进行分区有序的示例。一个订单的顺序流程是：创建、付款、推送、完成。订单号相同的消息会被先后发送到==同一个队列==中，消费时，同一个OrderId获取到的肯定是同一个队列。

![image-20220702222138374](RocketMQ-01.assets/image-20220702222138374.png)
...
public class Producer {
    public static void main(String[] args) throws MQClientException, InterruptedException {
        ...
        producer.setNamesrvAddr("10.211.55.13:9876;10.211.55.12:9876");
        //生产者这是监听器
